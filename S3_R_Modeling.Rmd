...
# stringAsFactors=False tells R to keep columns with text as categorical variables
...
```{r, eval=TRUE}
summary(indexes) # gives you descriptive statistics for each variable
```
...
# str shows you the types of data, cut
```
...
# to run commands on certain variables, do command$variable
# table is only useful with categorical variables
```
...
```{r, eval=TRUE}
explanans=names(indexes)[c(3:5)] #explanans is the elements of variables

for (x in explanans){ #for each x in explanans, compute the pearson coefficient between that x & environment
    p=cor.test(indexes[,x],indexes$environment)
    
    if(p$p.value<0.05){ # if p-value is less than 0.05, write this message
        messageYES=paste('environment index is correlated with',x)
        print(messageYES)
        
    }else{
        messageNO=paste('environment index is NOT correlated with',x) #if p-value is not <0.05, do this
        print(messageNO)
    }
    
}

# in R, everything needs to be in parentheses
# explanans includes all variables from 3:5 
```
```{r}
cor(indexes[,c(3:6)]) # all the rows, only columns 3:6
```
...
#creates LinRegEPI, which stores the results for the linear regression
# lm is the command to regress environment
```
